...
<input id="name" placeholder="الاسم الكامل (عند إنشاء حساب)">
...
async function login(){try{const d=await api('/auth/login',{method:'POST',body:JSON.stringify({identifier:identifier.value,password:password.value})});localStorage.setItem('token',d.token);closeAuth();alert('مرحبًا بعودتك، '+d.user.full_name)}catch(e){alert(e.error||'تعذر تسجيل الدخول')}}
async function registerUser(){const isEmail=identifier.value.includes('@');try{const d=await api('/auth/register',{method:'POST',body:JSON.stringify({full_name:name.value,password:password.value,email:isEmail?identifier.value:undefined,phone:!isEmail?identifier.value:undefined})});localStorage.setItem('token',d.token);closeAuth();alert('تم إنشاء الحساب: '+d.user.full_name)}catch(e){alert(e.error||'تعذر إنشاء الحساب')}}
